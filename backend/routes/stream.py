...
from db import Chat, Message, SessionLocal, User, now
from deps import (
    current_user,
    free_replies_today,
    get_owned_chat,
...
def prepare_turn(user_id, req):
    ...
    with SessionLocal() as db:
        ...
        system_prompt = llm.build_system_prompt(user.reply_language_mode, user.reply_language)
        ...
        return chat.id, chat.title, human.id, history, api_key, system_prompt, needs_summary


def finish_turn(chat_id, human_id, reply, model_id):
...
async def chat_stream(req: ChatRequest, user=Depends(current_user)):
    chat_id, title, human_id, history, api_key, system_prompt, needs_summary = (
    ...
    async def events():
        ...
        try:
            ...
            result = finish_turn(chat_id, human_id, "".join(parts), req.model)
